...
export const useAI = (): AIHooks => {
  ...
  const executeAIRequest = useCallback(
    async (requestFn: () => Promise<AIResponse>): Promise<AIResponse | null> => {
      setLoading(true);
      setError(null);
  ...
  const askQuestion = useCallback(
    async (question: string, context?: any): Promise<AIResponse | null> => {
      return executeAIRequest(() => aiIntegrationService.generateResponse(question, context));
    },
    [executeAIRequest]
  );

  const getPregnancyAdvice = useCallback(
    async (week: number, question: string): Promise<AIResponse | null> => {
      return executeAIRequest(() => aiIntegrationService.getPregnancyAdvice(week, question));
    },
    [executeAIRequest]
  );

  const getPostpartumSupport = useCallback(
    async (question: string, babyAge?: string): Promise<AIResponse | null> => {
      return executeAIRequest(() => aiIntegrationService.getPostpartumSupport(question, babyAge));
    },
    [executeAIRequest]
  );

  const searchWebForInfo = useCallback(
    async (query: string): Promise<AIResponse | null> => {
      return executeAIRequest(() => aiIntegrationService.searchWebForInfo(query));
    },
    [executeAIRequest]
  );

  const getEmotionalSupport = useCallback(
    async (message: string, context?: any): Promise<AIResponse | null> => {
      return executeAIRequest(() => aiIntegrationService.getEmotionalSupport(message, context));
    },
    [executeAIRequest]
  );

  const getNutritionAdvice = useCallback(
    async (question: string, context?: any): Promise<AIResponse | null> => {
      return executeAIRequest(() => aiIntegrationService.getNutritionAdvice(question, context));
    },
    [executeAIRequest]
  );
  ...
  const getSleepAdvice = useCallback(
    async (question: string, babyAge?: string): Promise<AIResponse | null> => {
      return executeAIRequest(() => aiIntegrationService.getSleepAdvice(question, babyAge));
    },
    [executeAIRequest]
  );
  ...
  const generateConversationSummary = useCallback(
    async (messages: string[]): Promise<AIResponse | null> => {
      return executeAIRequest(() => aiIntegrationService.generateConversationSummary(messages));
    },
    [executeAIRequest]
  );
...
export const useNathIA = () => {
  ...
  const sendMessage = useCallback(
    async (message: string, context?: any): Promise<string> => {
