...
export const useUserStats = () => {
    ...
    const getBadgeInfo = useCallback((badgeId: string) => {
        const badgeMap: Record<string, { name: string; description: string; icon: string }> = {
            first_task: { name: 'Primeira Tarefa', description: 'Complete sua primeira tarefa', icon: '⭐' },
            daily_complete: { name: 'Dia Completo', description: 'Complete todas as tarefas do dia', icon: '🏆' },
            week_streak: { name: 'Semana de Fogo', description: 'Mantenha o streak por 7 dias', icon: '🔥' },
            level_5: { name: 'Experiente', description: 'Alcance o nível 5', icon: '👑' }
        };
        return badgeMap[badgeId] || { name: 'Badge', description: 'Conquista desbloqueada', icon: '🏅' };
    }, []);
