...
export const EssenceOnboardingProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const setEmotionalState = useCallback((state: EmotionalState) => {
    ...
    setSteps((prev) =>
      prev.map((step) => (step.step === 1 ? { ...step, isCompleted: true } : step))
    );
  }, []);
  ...
  const setSelectedArchetype = useCallback((archetype: FeminineArchetype) => {
    ...
    setSteps((prev) =>
      prev.map((step) => (step.step === 2 ? { ...step, isCompleted: true } : step))
    );
  }, []);
  ...
  const getPersonalizedMessage = useCallback(() => {
    ...
    const messages = {
      guerreira:
        'Você não precisa ser forte o tempo todo. Aqui, a força materna é construída em silêncio, com amor e propósito. Vamos começar.',
      resiliente:
        'Sua capacidade de transformar cada desafio em sabedoria materna é sua maior força. Aqui, honramos sua jornada.',
      visionaria:
        'Sua visão pode transformar o futuro dos seus filhos. Aqui, damos vida aos seus sonhos de família.',
      cuidadora:
        'O amor que você oferece à sua família é sagrado. Aqui, cuidamos de quem cuida com tanto amor.',
    };
  ...
  return (
    <EssenceOnboardingContext.Provider value={value}>{children}</EssenceOnboardingContext.Provider>
  );
};
