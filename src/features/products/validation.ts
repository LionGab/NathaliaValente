 ...
 */
const ratingInfoSchema = z.object({
 ...
 */
const babyProductInfoSchema = z.object({
  ...
  careInstructions: z.string().max(500, 'Instruções de cuidado devem ter no máximo 500 caracteres'),
...
export const productSchema = z.object({
  ...
  category: z.enum(Object.values(PRODUCT_CATEGORIES) as [string, ...string[]], {
    errorMap: () => ({ message: 'Categoria inválida. Escolha entre: roupas, cuidados, acessórios, brinquedos, maternidade, nutrição, segurança' })
  }),
  ...
  tags: z.array(
    z.string()
      ...
      .regex(/^[a-zA-Z0-9\s\-_áàâãéèêíìîóòôõúùûçÁÀÂÃÉÈÊÍÌÎÓÒÔÕÚÙÛÇ]+$/, 'Tag contém caracteres inválidos')
  )
  ...
  seoKeywords: z.array(z.string().max(50))
    .max(10, 'Máximo 10 palavras-chave SEO')
    .optional()
});
