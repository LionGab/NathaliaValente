...
describe('ProductCard', () => {
  ...
  it('deve renderizar informaÃ§Ãµes do produto corretamente', () => {
    render(<ProductCard product={mockProduct} />);
    
    expect(screen.getByText(mockProduct.name)).toBeInTheDocument();
    expect(screen.getByText(R$ )).toBeInTheDocument();
    expect(screen.getByText(mockProduct.description)).toBeInTheDocument();
    expect(screen.getByText(${mockProduct.rating} â­)).toBeInTheDocument();
  ...
  it('deve chamar addToCart quando botÃ£o "Adicionar" Ã© clicado', () => {
    render(<ProductCard product={mockProduct} />);
  ...
  it('deve chamar removeFromCart quando produto jÃ¡ estÃ¡ no carrinho', () => {
  ...
  it('deve desabilitar botÃ£o quando produto estÃ¡ esgotado', () => {
    const outOfStockProduct = { ...mockProduct, stock: 0 };
    render(<ProductCard product={outOfStockProduct} />);
