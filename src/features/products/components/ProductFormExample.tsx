...
export const ProductFormExample: React.FC<ProductFormExampleProps> = ({
  ...
  const validateFormPreview = () => {
    ...
    if (result.success) {
      ...
      setValidationHistory((prev) => [
        ...prev,
        `❌ Formulário inválido: ${result.errors.length} erro(s)`,
      ]);
    }
          ...
          <div className="bg-gray-50 rounded-lg p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 text-xs">
                <div
                  className={`flex items-center gap-2 ${fieldErrors.name ? 'text-red-600' : 'text-green-600'}`}
                >
                  <span>{fieldErrors.name ? '❌' : '✅'}</span>
                  <span>Nome</span>
                </div>
                <div
                  className={`flex items-center gap-2 ${fieldErrors.price ? 'text-red-600' : 'text-green-600'}`}
                >
                  <span>{fieldErrors.price ? '❌' : '✅'}</span>
                  <span>Preço</span>
                </div>
                <div
                  className={`flex items-center gap-2 ${fieldErrors.currentStock ? 'text-red-600' : 'text-green-600'}`}
                >
                  <span>{fieldErrors.currentStock ? '❌' : '✅'}</span>
                  <span>Estoque</span>
                </div>
                <div
                  className={`flex items-center gap-2 ${fieldErrors.description ? 'text-red-600' : 'text-green-600'}`}
                >
            ...
            <div className="space-y-2">
              ...
              <div className="text-xs space-y-1">
                ...
                <div>
                  Status: {Object.keys(fieldErrors).length === 0 ? '✅ Válido' : '❌ Inválido'}
                </div>
              </div>
            </div>
          </div>
...
export const ProductFormExampleUsage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8">
      <ProductFormExample onSubmit={handleSubmit} onCancel={handleCancel} isEditing={false} />
    </div>
  );
};
