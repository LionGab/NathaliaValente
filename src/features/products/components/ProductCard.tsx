...
export const ProductCard: React.FC<ProductCardProps> = ({
    ...
    const getDiscountPercentage = () => {
        if (validatedProduct.originalPrice && validatedProduct.originalPrice > validatedProduct.price) {
            const discount = ((validatedProduct.originalPrice - validatedProduct.price) / validatedProduct.originalPrice) * 100;
            ...
            <div className="p-4">
                ...
                <div className="mb-3">
                    <div className="flex items-center gap-2">
                        ...
                        {validatedProduct.originalPrice && validatedProduct.originalPrice > validatedProduct.price && (
                ...
                {validatedProduct.averageRating && validatedProduct.totalReviews && (
                    <div className="flex items-center gap-1 mb-3">
                        <div className="flex text-yellow-400">
                            {[...Array(5)].map((_, i) => (
                                <span key={i}>
                                    {i < Math.floor(validatedProduct.averageRating!) ? '★' : '☆'}
                                </span>
                            ))}
                        </div>
                        <span className="text-sm text-gray-600">
                            ({validatedProduct.totalReviews})
                        </span>
                    </div>
                )}
                ...
                <div className="text-sm text-gray-600 mb-3">
                    {inStock ? (
                        <span className="text-green-600">
                            {validatedProduct.currentStock} em estoque
                        </span>
                    ) : (
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        disabled={!canAdd}
                        className={`flex-1 px-3 py-2 text-sm rounded-lg transition-colors ${canAdd
