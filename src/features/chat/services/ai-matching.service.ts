...
class AIMatchingService {
  ...
  async findMatches(
    ...
    try {
      ...
      const sortedMatches = matchesWithScores.sort((a, b) => b.score - a.score).slice(0, limit);
    ...
    try {
      const demoProfile: Partial<UserProfile> = {
        ...
        goals: [
          ...
          'Aprender sobre maternidade',
        ],
      };

      const { data, error } = await supabase.from('profiles').insert(demoProfile).select().single();
  ...
  private async calculateCompatibilityScore(
    ...
    const compatibility = {
      ...
      motherhood_stage: this.calculateMotherhoodStageCompatibility(
        user.motherhood_stage,
        match.motherhood_stage
      ),
    ...
    const score = Object.entries(compatibility).reduce((total, [key, value]) => {
      return (
        total + value * this.COMPATIBILITY_WEIGHTS[key as keyof typeof this.COMPATIBILITY_WEIGHTS]
      );
    }, 0);
  ...
  private calculateMotherhoodStageCompatibility(userStage: string, matchStage: string): number {
    ...
    return (
      stageCompatibility[userStage as keyof typeof stageCompatibility]?.[
        matchStage as keyof (typeof stageCompatibility)[typeof userStage]
      ] || 0
    );
  }
