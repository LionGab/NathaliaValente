...
export const NathInspiraCard: React.FC<NathInspiraCardProps> = ({
    ...
    return (
        <article className={`relative overflow-hidden rounded-3xl ${config.bgColor} ${config.borderColor} border-2 transition-all duration-300 hover:shadow-xl hover:scale-[1.02] group`}>
            {/* Gradient Background */}
            ...
            <div className="relative px-6 py-4 border-t border-neutral-200/50 dark:border-neutral-700/50">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-6">
                        <button
                            onClick={handleLike}
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-200 ${post.isLiked
                                    ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'
                                    : 'hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-600 dark:text-neutral-400'
                                }`}
                        >
                            <Heart
                                className={`w-5 h-5 transition-all duration-200 ${post.isLiked ? 'fill-current scale-110' : ''
                    ...
                    >
                        <Bookmark
                            className={`w-5 h-5 transition-all duration-200 ${post.isBookmarked ? 'fill-current' : ''
                                }`}
                        />
                    </button>
                </div>
