...
export const EnhancedRoutinePreview: React.FC<EnhancedRoutinePreviewProps> = ({ onViewAll }) => {
  ...
  const getMotivationalMessage = () => {
    const completionRate =
      todayRoutines.length > 0 ? (completedCount / todayRoutines.length) * 100 : 0;
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-xs font-bold text-blue-600 dark:text-blue-400">
              {todayRoutines.length > 0
                ? Math.round((completedCount / todayRoutines.length) * 100)
                : 0}
              %
            </span>
          </div>
      ...
      >
        ...
        {todayRoutines.length > 0 && (
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span>
              {completedCount}/{todayRoutines.length} concluídas
            </span>
          </div>
        )}
      </motion.div>
