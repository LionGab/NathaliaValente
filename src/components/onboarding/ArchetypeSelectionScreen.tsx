...
export const ArchetypeSelectionScreen: React.FC = () => {
  ...
  const handlePlayAudio = useCallback(
    (archetype: FeminineArchetype, event: React.MouseEvent) => {
    ...
    <div className="text-center space-y-4 sm:space-y-6">
      ...
      >
        <motion.div
          ...
          className="relative w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-accent-500 to-secondary-600 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4 maternal-shadow-lg"
        >
          ...
          <motion.div
            ...
            className="absolute inset-0 rounded-full bg-gradient-to-br from-accent-500 to-secondary-600 animate-ping opacity-30"
          />
        </motion.div>
      ...
      >
        {FEMININE_ARCHETYPES.map((archetype, index) => (
          <motion.div
            ...
            whileTap={{ scale: 0.98 }}
            className={`group relative p-6 sm:p-8 rounded-3xl text-left transition-all duration-500 cursor-pointer overflow-hidden touch-target ${
            ...
            <motion.button
              ...
              aria-label={
                playingAudio === archetype.id
                  ? `Parar áudio de ${archetype.label}`
                  : `Tocar áudio de ${archetype.label}`
              }
              tabIndex={0}
            >
              {playingAudio === archetype.id ? (
      ...
      >
        ...
        <motion.button
          ...
          className={`flex-1 py-4 px-6 sm:px-8 rounded-2xl font-semibold text-base sm:text-lg transition-all duration-300 touch-target flex items-center justify-center gap-2 ${
            selectedArchetype
          ...
          aria-label={
            selectedArchetype
              ...
              : 'Selecione um arquétipo para continuar'
          }
        ...
        >
          <div className="inline-flex items-center gap-2 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 px-4 py-2 rounded-full text-sm font-medium">
            ...
            <span>
              ...
              {FEMININE_ARCHETYPES.find((a) => a.id === selectedArchetype)?.label}
            </span>
          </div>
        </motion.div>
      )}
