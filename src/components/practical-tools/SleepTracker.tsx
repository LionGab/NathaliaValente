...
export const SleepTracker: React.FC = () => {
  ...
  const getNapCount = () => {
    ...
    return sessions.filter((session) => session.startTime >= today && session.type === 'nap')
      .length;
  };
  ...
  const getSleepQuality = () => {
    ...
    const qualityCounts = recentSessions.reduce(
      (acc, session) => {
        if (session.quality) {
  ...
  const getSleepStatus = () => {
    ...
    if (babyAge < 30) {
      ...
      if (totalSleep > 18 * 60)
        return { status: 'info', message: 'Muito sono - verifique se está bem' };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-xl font-bold text-gray-800 dark:text-white">Cronômetro de Sono</h3>
          <p className="text-gray-600 dark:text-gray-400">Acompanhe o sono do seu bebê</p>
        </div>
      </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 mb-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 dark:text-blue-300">{getSleepRecommendations()}</p>
          </div>
        </div>
      </div>
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`w-2 h-2 rounded-full ${
                      session.type === 'nap' ? 'bg-yellow-500' : 'bg-indigo-500'
                    }`}
                  ></div>
