...
export const MotherWellnessTracker: React.FC = () => {
  ...
  const getWeeklyAverage = (field: keyof WellnessEntry) => {
    ...
    const recentEntries = entries.filter((entry) => new Date(entry.date) >= weekAgo);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-xl font-bold text-gray-800 dark:text-white">Bem-estar da Mãe</h3>
          <p className="text-gray-600 dark:text-gray-400">Acompanhe seu autocuidado e bem-estar</p>
        </div>
      </div>
          ...
          {goals.map((goal) => (
            <div key={goal.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="font-medium text-gray-800 dark:text-white">{goal.title}</span>
              ...
              <div key={entry.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <span className="text-indigo-600 dark:text-indigo-400">😴 {entry.sleep}h</span>
                  </div>
                </div>
                {entry.notes && (
                  <p className="text-xs text-gray-600 dark:text-gray-400 italic">"{entry.notes}"</p>
                )}
              </div>
            ))}
          </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEntry((prev) => ({ ...prev, mood: Number(e.target.value) }))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEntry((prev) => ({ ...prev, energy: Number(e.target.value) }))
                  }
                  className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEntry((prev) => ({ ...prev, sleep: Number(e.target.value) }))
                  }
                  className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEntry((prev) => ({ ...prev, stress: Number(e.target.value) }))
                  }
                  className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewEntry((prev) => ({ ...prev, selfCare: Number(e.target.value) }))
                  }
                  className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                />
              </div>
