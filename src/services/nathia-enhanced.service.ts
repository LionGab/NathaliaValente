...
const analyzeEmotionalState = (
  message: string,
  history: any[]
): NathIAContext['emotionalState'] => {
  const messageLower = message.toLowerCase();

  // Palavras que indicam ansiedade
  if (
    messageLower.includes('ansiosa') ||
    messageLower.includes('preocupada') ||
    messageLower.includes('nervosa') ||
    messageLower.includes('medo')
  ) {
    return 'anxious';
  }
  ...
  if (
    messageLower.includes('sobrecarregada') ||
    messageLower.includes('cansada') ||
    messageLower.includes('exausta') ||
    messageLower.includes('não aguento')
  ) {
    return 'overwhelmed';
  }
  ...
  if (
    messageLower.includes('frustrada') ||
    messageLower.includes('raiva') ||
    messageLower.includes('irritada') ||
    messageLower.includes('não consigo')
  ) {
    return 'frustrated';
  }
  ...
  if (
    messageLower.includes('feliz') ||
    messageLower.includes('orgulhosa') ||
    messageLower.includes('conquista') ||
    messageLower.includes('sucesso')
  ) {
    return 'happy';
  }
 ...
 */
const generatePersonalizedResponse = (message: string, context: NathIAContext): NathIAResponse => {
  const messageLower = message.toLowerCase();
  ...
  if (!validateMaternityContext(message)) {
    return {
      message:
        'Querida, sou especializada em apoiar mães em sua jornada! 💕 Posso te ajudar com questões sobre maternidade, desenvolvimento infantil, rotinas familiares, bem-estar materno, ou até mesmo momentos espirituais. Como posso te apoiar hoje? 🌟',
  ...
  if (
    messageLower.includes('birra') ||
    messageLower.includes('birras') ||
    messageLower.includes('comportamento')
  ) {
    return generateBehavioralResponse(message, context);
  }

  if (
    messageLower.includes('sono') ||
    messageLower.includes('dormir') ||
    messageLower.includes('insônia')
  ) {
  ...
  if (
    messageLower.includes('rotina') ||
    messageLower.includes('organizar') ||
    messageLower.includes('cronograma')
  ) {
    return generateRoutineResponse(message, context);
  }

  if (
    messageLower.includes('alimentação') ||
  ...
  if (
    messageLower.includes('culpa') ||
    messageLower.includes('culpada') ||
    messageLower.includes('erro')
  ) {
    return generateGuiltResponse(message, context);
  }

  if (
    messageLower.includes('bíblico') ||
    messageLower.includes('estudo') ||
    messageLower.includes('bíblia')
  ) {
  ...
  if (
    messageLower.includes('oração') ||
    messageLower.includes('orar') ||
    messageLower.includes('rezar')
  ) {
    return generatePrayerResponse(message, context);
  }

  if (
    messageLower.includes('reflexão') ||
    messageLower.includes('espiritual') ||
    messageLower.includes('fé')
  ) {
...
const generateBehavioralResponse = (message: string, context: NathIAContext): NathIAResponse => {
  ...
  if (hasDiscussedBehavior) {
    response =
      'Vejo que as birras têm sido um tema recorrente para vocês. 💙 Isso é totalmente normal! Cada criança tem seu próprio ritmo. Que tal tentarmos uma abordagem diferente hoje?';
    practicalTips = [
    ...
    response =
      'As birras são uma fase natural do desenvolvimento! 💫 Aqui estão estratégias que realmente funcionam:';
    practicalTips = [
...
const generateSleepResponse = (message: string, context: NathIAContext): NathIAResponse => {
  ...
  if (hasDiscussedSleep) {
    response =
      'O sono continua sendo um desafio? 😴 Vamos revisar nossa estratégia! Como tem sido a rotina desde nossa última conversa?';
    practicalTips = [
    ...
    response =
      'O sono dos pequenos pode ser um desafio, mas temos estratégias que funcionam! 😴 Vamos criar uma rotina personalizada:';
    practicalTips = [
 ...
 */
const generateEmotionalSupportResponse = (
  message: string,
  context: NathIAContext
): NathIAResponse => {
  const response =
    'Querida, é normal se sentir assim! 🤗 A maternidade é intensa e você está fazendo um trabalho incrível. Vamos cuidar de você também:';
...
const generateRoutineResponse = (message: string, context: NathIAContext): NathIAResponse => {
  ...
  if (hasDiscussedRoutine) {
    ...
    response =
      'Organizar a rotina é um superpoder materno! 📅 Vamos criar algo que funcione para vocês:';
    practicalTips = [
...
const generateGuiltResponse = (message: string, context: NathIAContext): NathIAResponse => {
  const response =
    'A culpa materna é um sentimento que muitas de nós conhecemos. 💙 Mas saiba que:';
...
const generateBiblicalResponse = (message: string, context: NathIAContext): NathIAResponse => {
  const response =
    "Que lindo momento espiritual! 🙏 Vamos refletir juntas: 'Ensina a criança no caminho em que deve andar, e ainda quando for velho não se desviará dele.' (Provérbios 22:6). A maternidade é um chamado sagrado.";

  return {
    message:
      response +
      ' Que tal começarmos com um estudo bíblico sobre paciência e amor incondicional? Como você tem ensinado valores cristãos para seu filho? 💕',
...
const generateScriptureResponse = (message: string, context: NathIAContext): NathIAResponse => {
  const response =
    "Deus tem palavras especiais para nós, mães! 📖 'Porque eu bem sei os pensamentos que tenho a vosso respeito, diz o Senhor; pensamentos de paz, e não de mal, para vos dar o fim que esperais.' (Jeremias 29:11). Você está no lugar certo, no tempo certo.";
...
const generatePrayerResponse = (message: string, context: NathIAContext): NathIAResponse => {
  const response =
    "Vamos orar juntas, querida! 🙏 'Senhor, abençoa esta mãe corajosa. Dá-lhe sabedoria para cada decisão, paciência para os momentos difíceis, e alegria para celebrar cada conquista. Que ela sinta Seu amor incondicional em cada dia. Fortalece-a para ser o exemplo de amor que seu filho precisa. Amém.'";
...
const generateSpiritualResponse = (message: string, context: NathIAContext): NathIAResponse => {
  const response =
    'Que momento especial de conexão! ✨ A maternidade é um reflexo do amor divino. Cada abraço, cada palavra de carinho, cada momento de paciência é um ato de amor. Você está sendo instrumento de Deus na vida do seu filho.';

  return {
    message:
      response + ' Que lindo chamado! Como você tem visto a mão de Deus na sua jornada materna? 🌈',
...
const generateContextualResponse = (message: string, context: NathIAContext): NathIAResponse => {
  ...
  if (context.emotionalState === 'anxious') {
    response =
      'Vejo que você está passando por momentos de ansiedade. 💙 Isso é normal na maternidade. Vamos trabalhar juntas para te trazer mais calma e confiança.';
  } else if (context.emotionalState === 'overwhelmed') {
    response =
      'Querida, é normal se sentir sobrecarregada. 🤗 A maternidade é intensa, mas você está fazendo um trabalho incrível. Vamos cuidar de você também.';
  } else if (context.emotionalState === 'frustrated') {
    response =
      'Entendo sua frustração. 💪 Ser mãe não é fácil, mas você está no caminho certo. Vamos encontrar estratégias que funcionem para vocês.';
  } else if (context.emotionalState === 'happy') {
    response =
      'Que lindo ver sua alegria! 🌟 A maternidade tem seus momentos mágicos, não é? Vamos celebrar essas conquistas juntas!';
  } else {
...
export const generateNathIAResponse = async (
  ...
  try {
    ...
    const previousTopics = chatHistory
      ...
      .split(' ')
      .filter((word) =>
        ['birra', 'sono', 'alimentação', 'rotina', 'culpa', 'bíblico', 'oração'].includes(word)
      );
    ...
    return {
      message:
        'Desculpe, estou passando por um momento técnico. Mas saiba que estou aqui para te apoiar! 💕 Como posso te ajudar hoje?',
